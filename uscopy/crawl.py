...
parser.add_argument('pattern', metavar='-p', type=str, help='Unix style-like pattern to be queried')
parser.add_argument('source', metavar='-s', type=str, help='Where to look for files that match the pattern')
parser.add_argument('destination', metavar='-d', type=str, help='where to accumulate found files')
parser.add_argument('dice', type=int, metavar='-D', help='a chosen file every dice', default=1)
parser.add_argument('omit', metavar='-o', type=str, help='unix style-like patterns as a space delimited string')
...
for i_match, match in enumerate(matches):
    ...
    if (current - log_time) / 10e-9 > 5:
        log_time = current
        print(f'Progress...: {int(i_match / total_matches * 100)}%', end="\r", flush=True)
