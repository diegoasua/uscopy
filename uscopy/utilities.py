...
def stack_single_tiffs(base_dir: str, remove_old=True, verbose=True) -> None:
    ...
    logger = []
    datafolders = [name for name in os.listdir(base_directory) if os.path.isdir(os.path.join(base_directory, name))] # clean anything which is not a folder
    for datafolder in tqdm.tqdm(datafolders):
        path_to_data = base_directory / datafolder
        dataset = []
        files = sorted(os.listdir(path_to_data)) # important: sort, otherwise os.listdir() returns a random order
        r = re.compile(".*tif")
        ...
        for n_file_name, file_name in enumerate(files):
            if this_file.ndim > 2:
                for piece in this_file:
                    dataset.append(piece) # first file contains 10 frames (a mystery) so we just take only the last one.
            else:
                dataset.append(this_file)
        try:
            dataset = np.stack(dataset)
        except ValueError:
            print(f'\x1b[31m\"Could not stack dataset: {datafolder}. Is the folder empty?\"\x1b[0m')
            continue
        ...
        logger.append(f"{datafolder}.tif \n\t data structure: {type(dataset)} \n\t shape: {dataset.shape} \n\t dtype: {dataset.dtype}\n")
